...
$id = $row['id'] ?? 0;
//create billing id
$bid = $site_code . date('Y') . 'BILL' . ($id + 1);
?>
<!DOCTYPE html>
...
<body>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="flex-wrap d-flex justify-content-between align-items-center">
                                ...
                                <div>
                                    <a href="<?= $admin_url ?>/pages/viewbilling/"><button type="button" class="btn btn-success">
                        ...
                        <div class="card">
                            <div class="row p-3 g-2 cform text-center">
                                <div class="col-12 col-lg-9 row g-2 text-center">
                                    ...
                                    <div class="col-12 col-sm-6 col-xxl-4 text-center">
                                        <input type="checkbox" name="gst" id="gst" class="gstbox" oninput="calc_gst()" />
                                        <label for="gst" class="gstbtn pt-1 text-white"></label>
                                    </div>
                                </div>
                                ...
                                <div class="table-responsive flex">
                                    ...
                                    <div>
                                        <table class="table table-striped" role="grid" data-bs-toggle="data-table">
                                            ...
                                            <tr class="packdiv">
                                                <td colspan="4" style="text-align:Right;">Packing Charge (%) : <input type="number" id="pach" name="pach" oninput="pcharge()" /></td>
                                                <td colspan="1">Charge (Rs) : </td>
                                                <td colspan="1"><input type="number" readonly value="0" id="fpach" name="fpach" /></td>
                                            </tr>
                                            <tr class="gstdiv">
                                                <td colspan="4"></td>
                                                <td colspan="1">GST (18%) (Rs) : </td>
                                                <td colspan="1"><input type="number" readonly value="0" id="fgst" name="fgst" /></td>
                                            </tr>
                                            <tr class="totaltr">
                                                <td colspan="4"></td>
    ...
    <script>
        ...
        const roamt = () => {
            ...
            let gst = parseFloat($("#fgst").val());
            let pamt = parseFloat($("#fpach").val());
            $("#roAmt").val(Math.round(total + gst + pamt));
        };
        const calc_gst = () => {
            ($("#addItems tr").length == 0) ? $(".gstdiv").hide(): ($("#gst").prop("checked")) ? $(".gstdiv").show() : $(".gstdiv").hide();
            let gst = ($("#gst").prop("checked")) ? 0.18 : 0;
            let total = $("#totalAmt").val();
            let gstamt = total * gst;
            $("#fgst").val(gstamt.toFixed(2));
            roamt();
        };
        const pcharge = () => {
        ...
        const addvalues = (pid) => {
            ...
            $("#itemid_" + itemid).val(item.alignment);
        ...
        const searchitem = () => {
            ...
            results.map((item) => {
                let li = `<li id="no_${itemid}" class="border-bottom border-1 border-secondary" onclick="addvalues(${item.id})">${item.alignment} - ${item.name}</li>`;
