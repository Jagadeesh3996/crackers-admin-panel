<?php
    ...
    function Add(){
        ...
        if (isset($_FILES['image'])) {
            ...
            if(!getimagesize($image_tmp_name)) {   // Check if image file is a actual image
                ...
                return;
            }else if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "webp") {  // Allow certain file formats
                echo "Sorry! only JPG, JPEG, PNG files only are allowed.";
                return;
            } else {
                if (move_uploaded_file($image_tmp_name, $target_path)) {
                    $query = "INSERT INTO tbl_instagram (url, image) VALUES ('$url', '$image_name')";
    ...
    function Delete(){
        ...
        $query2 = "UPDATE tbl_instagram SET status = 0 WHERE id = '$id'";
    ...
    function Fetch(){
        ...
        $query3 = "SELECT * FROM tbl_instagram WHERE id = '$id' and status = '1'";
    ...
    function Edit(){
        ...
        if (isset($_FILES['editImage'])) {
            ...
            if(!getimagesize($editimage_tmp_name)) {   // Check if image file is a actual image
                ...
                echo "Sorry! File is too large.";
                return;
            }else if($editimageFileType != "jpg" && $editimageFileType != "png" && $editimageFileType != "jpeg" && $editimageFileType != "gif" && $imageFileType != "webp") {  // Allow certain file formats
                echo "Sorry! only JPG, JPEG, PNG & GIF files are allowed.";
                return;
            } else {
                if (move_uploaded_file($editimage_tmp_name, $target_path)) {
                    $query4 = "UPDATE tbl_instagram SET url='$url', image ='$editimage_name' WHERE id = '$id'";
                } else {
                    echo "Sorry, there was an error in updating this data.";
                }
            }
        } else {
            $query4 = "UPDATE tbl_instagram SET url='$url' WHERE id = '$id'";
        };
